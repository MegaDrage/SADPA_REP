...
const int N = 4000;  // Количество записаей в БД
const int M = 256;
int sum = 0;
int sym_count = 0;
int code[M][M];
...
int main() {
    ...
    while (true) {
        ...
        //		SetConsoleCP(866);
        cout << "\t------------------------------MENU------------------------" << endl;
        cout << "\t1. Unsort DB" << endl;
        cout << "\t2. Sorted DB (Heap sort)" << endl;
        cout << "\t3. Bin serch in DB (year)" << endl;
        cout << "\t4. Bin serch in DB (author surname)" << endl;
        cout << "\t5. Coding" << endl;
        cout << "\tEsc. Exit " << endl;
        cout << "\t----------------------------------------------------------" << endl;
        ...
        switch (enter) {
            ...
            case 51:
                ...
                if (search == -1) {
                    ...
                    cout << endl
                         << endl
                         << "Found " << search - search_start << " records: " << search_start << " - "
                         << search - 1 << endl;
                    //				SetConsoleCP(1251);
                    for (int i = search_start; i < search; i++) {
                        cout << i + 1 << "  " << mas[i]->author << "  " << mas[i]->title << "  "
                             << mas[i]->publisher << "  " << mas[i]->year << "  " << mas[i]->num_of_page
                             << endl;
                    }
...
//_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /_ВЫВОД_/ /
void Print_base(record* mas) {
    int i;
    char n;
    //	SetConsoleCP(1251);
    ...
    if (n == '1') {
        i = 0;
        while (i < N) {
            cout << i << "  " << mas[i].author << "  " << mas[i].title << "  " << mas[i].publisher << "  "
                 << mas[i].year << "  " << mas[i].num_of_page << endl;
            i++;
    ...
    if (n == '2') {
        ...
        for (int i = 0; i < N; i = i + 20) {
            while ((g++) < 20) {
                cout << i + g << "  " << mas[i + g].author << "  " << mas[i + g].title << "  "
                     << mas[i + g].publisher << "  " << mas[i + g].year << "  " << mas[i + g].num_of_page
                     << endl;
            }
            if (g >= 20) {
...
// void Down(int n, int j) {
//     int S[M];
//     for (int i = 0; i < M; i++) S[i] = code[j][i];
//     int L = A[j].l;
//     for (int i = j; i < n - 1; i++) {
//         for (int k = 0; k < M; k++) code[i][k] = code[i + 1][k];
//         A[i].l = A[i + 1].l;
//     }
//     for (int i = 0; i < M; i++) {
//         code[n - 1][i] = S[i];
...
//     lgm = 0;
//     //	SetConsoleCP(866);
//     printf("\n\nOptimal code of Hafman: \n");
//     printf("-------------------------------------------------------------------------------\n");
//     printf("|  Simbol num   | Simbol | Probability |       Code word      | Code word len |\n");
//     printf("|-----------------------------------------------------------------------------|\n");
//     //	SetConsoleCP(1251);
//     for (int i = 0; i < M && A[i].p != 0; i++) {
//         printf("|  %12d |    %c   |  %2.6f   | ", i, A[i].a, A[i].q);
//         for (int j = 0; j < A[i].l + 1; j++) printf("%d", code[i][j]);
...
void LRprint(vertex* x) {
    ...
    if (x) {
        LRprint(x->left);
        cout << "  " << x->data->author << "  " << x->data->title << "  " << x->data->publisher << "  "
             << x->data->year << "  " << x->data->num_of_page << endl;
        LRprint(x->next);
        LRprint(x->right);
    }
}
    ...
    if (p != NULL)  /// Поиск элемента
    {
        ...
        if (check > 0) {
            ...
            if (check < 0) {
                ...
                cout << "  " << p->data->author << "  " << p->data->title << "  " << p->data->publisher
                     << "  " << p->data->year << "  " << p->data->num_of_page << endl;
...
int compare(record* AR, record* BR) {
    char bufferA[12];
    char bufferB[12];
    strncpy(bufferA, AR->author, 3);  // Copy the first three characters of the surname
    bufferA[3] = '\0';                // Null-terminate the string
    strncpy(bufferB, BR->author, 3);
    bufferB[3] = '\0';
...
int compare_auth(char* AR, char* BR) {
    char bufferA[12];
    char bufferB[12];
    strncpy(bufferA, AR, 3);  // Copy the first three characters of the surname
    bufferA[3] = '\0';        // Null-terminate the string
    strncpy(bufferB, BR, 3);
    bufferB[3] = '\0';

    return strcmp(bufferA, bufferB);
}

//_СОРТИРОВКА_/ /_СОРТИРОВКА_/ /_СОРТИРОВКА_/ /_СОРТИРОВКА_/ /_СОРТИРОВКА_/ /_СОРТИРОВКА_/ /_СОРТИРОВКА_/ /
void HeapBuild(record** A, int l, int r) {
    int i = l, j;
    record* x = A[l];
...
//_КОДИРОВАНИЕ_/ /_КОДИРОВАНИЕ_/ /_КОДИРОВАНИЕ_/ /_КОДИРОВАНИЕ_/ /_КОДИРОВАНИЕ_/ /_КОДИРОВАНИЕ_/
///_КОДИРОВАНИЕ_/ /
